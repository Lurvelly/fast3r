...
from fast3r.dust3r.utils.image import _resize_pil_image
from fast3r.dust3r.utils.device import to_numpy
from fast3r.scene.colmap_loader import (
...
def save_extrinsic(sparse_path, extrinsics_w2c, img_files, image_suffix):
    ...
    images_txt_file = os.path.join(sparse_path, 'images.txt')
    images = {}
...
def save_intrinsics(sparse_path, focals, org_imgs_shape, imgs_shape, save_focals=False):
    ...
    cameras_txt_file = os.path.join(sparse_path, 'cameras.txt')
    ...
    if save_focals:
        np.save(os.path.join(sparse_path, 'non_scaled_focals.npy'), focals)
...
def save_points3D(sparse_path, imgs, pts3d, confs, masks=None, use_masks=True, save_all_pts=False, save_txt_path=None, depth_threshold=0.1, max_pts_num=150 * 10**10):
    ...
    points3D_txt_file = os.path.join(sparse_path, 'points3D.txt')
    points3D_ply_file = os.path.join(sparse_path, 'points3D.ply')
...
def export_combined_ply(preds, views, export_ply_path=None, 
    ...
    export_ply_path = os.path.join(export_ply_path, 'points3D.ply')
    ...
    all_normals = np.zeros_like(all_colors)
    # Export as .ply if the path is provided
    if export_ply_path:
        point_cloud = trimesh.Trimesh(vertices=all_points, vertex_colors=all_colors, vertex_normals=all_normals)
